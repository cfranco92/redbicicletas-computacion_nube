...
export class BicycleRentingComponent implements OnInit {
  ...
  getBicycles() {
    this.rentingService.read().subscribe((data) => {
      this.bicycles = data.body.map((bicycle: any) =>
  ...
  rentBicycleById(bicycle: Bicycle) {
    ...
    this.rentingService.updateById(bicycle).subscribe((response: any) => {
      alert(
        `Bicleta con id ${bicycle.bicycleId} ha sido rentada por ${this.user.displayName} con el id ${this.user.uid}`
      );
    });
  }
  ...
  returnBicycleById(bicycle: Bicycle) {
    ...
    this.rentingService.updateById(bicycle).subscribe((response: any) => {
      alert(
        `Bicleta con id ${bicycle.bicycleId} ha sido entregada por ${this.user.displayName} con el id ${this.user.uid}`
      );
    });
  }
}
